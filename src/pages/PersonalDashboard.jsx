...
export default function PersonalDashboard() {
  ...
  // 1. Cargar lista de transportistas si no soy uno
  useEffect(() => {
  ...
  // 2. Cargar Datos del Dashboard según filtros
  useEffect(() => {
    ...
    fetchDashboardData();
  }, [profile, year, month, fechaInicio, fechaFin, selectedTransportista, isTransportista, isAdminOrDev]);
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
        ...
        {/* Filtro de Transportista (Oculto para el operador logístico) */}
        {!isTransportista && (
          <div className="lg:col-span-2">
            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1 flex items-center gap-1">
              <User className="w-3 h-3" /> Ver Transportista Específico
            </label>
            <select 
              className="w-full h-11 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 px-3 text-sm font-medium dark:text-white focus:ring-2 focus:ring-brand-500 outline-none"
              value={selectedTransportista}
              onChange={(e) => setSelectedTransportista(e.target.value)}
            >
              <option value="">-- Mi Gestión Global --</option>
      ...
      {loading || !data ? (
        <div className="text-center py-10 text-gray-500 font-medium animate-pulse">Calculando métricas...</div>
      ) : (
        <>
          {/* 3. Tarjetas de KPIs */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {/* Relojes */}
            <div className="lg:col-span-4 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col justify-around space-y-2">
                    ...
                    <table className="w-full text-sm text-left">
                      <thead className="text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-slate-900 uppercase border-b border-gray-200 dark:border-slate-700">
                        <tr>
                          ...
                          <th className="px-4 py-2 font-bold text-right">Recuento de Validación</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100 dark:divide-slate-700 text-gray-800 dark:text-gray-200">
                         <tr>
                            <td className="px-4 py-3 flex items-center gap-2">
                               <span className="w-2 h-2 rounded-full bg-green-500"></span> A tiempo
                            </td>
                            <td className="px-4 py-3 text-right font-medium">{data.categorias?.aTiempo || 0}</td>
                         </tr>
                         <tr>
                            <td className="px-4 py-3 flex items-center gap-2">
                               <span className="w-2 h-2 rounded-full bg-red-500"></span> Desfase
                            </td>
                            <td className="px-4 py-3 text-right font-medium">{data.categorias?.desfase || 0}</td>
                         </tr>
                         <tr className="bg-gray-50/50 dark:bg-slate-900/50 font-black text-gray-900 dark:text-white">
                    ...
                    <table className="w-full text-sm text-left">
                      <thead className="text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-slate-900 uppercase border-b border-gray-200 dark:border-slate-700">
                        <tr>
                          ...
                          <th className="px-4 py-2 font-bold text-right">Aprobado</th>
                          <th className="px-4 py-2 font-bold text-right">Pagado</th>
                          <th className="px-4 py-2 font-bold text-right">Rechazado</th>
                      ...
                      <tbody className="divide-y divide-gray-100 dark:divide-slate-700 text-gray-800 dark:text-gray-200">
                         {data.tablaValidacion?.length > 0 ? data.tablaValidacion.map(v => (
                            <tr key={v.nombre} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                               ...
                               <td className="px-4 py-3 text-right text-green-600 dark:text-green-400 font-medium">{v.aprobado}</td>
                               <td className="px-4 py-3 text-right text-blue-600 dark:text-blue-400 font-medium">{v.pagado}</td>
                               <td className="px-4 py-3 text-right text-red-600 dark:text-red-400 font-medium">{v.rechazado}</td>
                               <td className="px-4 py-3 text-right font-bold text-gray-900 dark:text-white">{v.total}</td>
                            </tr>
                         )) : (
                            <tr><td colSpan="5" className="px-4 py-6 text-center text-gray-400">Sin datos de validación.</td></tr>
                         )}
                ...
                {data.grafico?.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.grafico} layout="vertical" margin={{ left: 40, right: 60 }}>
                      ...
                      <YAxis dataKey="name" type="category" width={100} tick={{fontSize: 11}} stroke="#94a3b8" axisLine={false} tickLine={false} />
                      <Tooltip 
